...
class Helpers
{
    ...
    {
        $diff = date_diff(date_create_immutable($date1), date_create_immutable($date2));
        ...
        switch ($interval) {
            ...
            case 'hours':
                ...
                $result = '';
                if ($hours > 0) {
                    $result .= sprintf('%d %s', $hours, 'hour'.($hours == 1 ? '' : 's'));
                }
                if ($minutes > 0) {
                    $result .= sprintf(' %d %s', $minutes, 'minute'.($minutes == 1 ? '' : 's'));
                }
                if ($seconds > 0) {
                    $result .= sprintf(' %d %s', $seconds, 'second'.($seconds == 1 ? '' : 's'));
                }
    ...
    public static function create_slug(string $string): string
    {
    ...
    public static function sanitize_url(string $inputURL): mixed
    {
    ...
    public static function is_valid_phone(string $phoneNumber): bool
    {
     ...
     * @throws \Exception
     */
    public function generate_idx(int $length = 9): string
    {
        ...
        $idx = substr(bin2hex($bytes), 0, $length);
    ...
    public static function create_directory(string $directoryPath, int $permissions = 0755): void
    {
    ...
    public static function cut_html_text(string $html, int $maxChar): string
    {
        $doc = new DOMDocument();
        ...
        foreach ($nodes as $node) {
            if ($node->nodeName === '#text') {
                ...
                $childText = self::cut_html_text($childHtml, $maxChar - $length);
                $text .= $childText;
    ...
    {
        define('IP_HEADERS', ['HTTP_CLIENT_IP', 'HTTP_X_FORWARDED_FOR', 'HTTP_X_FORWARDED', 'HTTP_X_CLUSTER_CLIENT_IP', 'HTTP_FORWARDED_FOR', 'HTTP_FORWARDED', 'REMOTE_ADDR']);
    ...
    public static function geo_ip(string $adresseIP): ?array
    {
        ...
        return ['country_name' => $country_name, 'city' => $city, 'latitude' => $latitude, 'longitude' => $longitude];
    }
     ...
     * @throws \Exception
     */
    public static function verif_date(string $date): bool
    {
    ...
    public static function csv_to_json(string $csv_file, string $json_file, string $delimiter = ','): bool
    {
    ...
    public static function csv_to_xml(string $csv_file, string $xml_file, string $root_element_name = 'data', string $record_element_name = 'record'): bool
    {
    ...
    public static function calculate_reading_time(string $text): string
    {
